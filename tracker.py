...
def render(rows, c, errors, demo, path, alerts):
    ...
    for s in c['searches']:
        ...
        searches.append(f'<tr><td>{esc(s["name"])}</td><td>{money(s["max_price"])}</td><td>{money(s["typical_used_price"])}</td><td><a href="{esc(url)}">Search eBay ↗</a> · <a href="{esc(local)}">Nearby ↗</a></td></tr>')
    status = 'DEMO · invented examples, no live listings' if demo else 'LIVE CHECK · ' + dt.datetime.now().astimezone().strftime('%b %d, %Y · %I:%M %p %Z')
    content = '''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="300"><title>Gear Scout · eBay deals</title><style>
:root{color-scheme:light}*{box-sizing:border-box}body{margin:0;background:#f5f4ed;color:#21372f;font:16px system-ui,sans-serif}main{max-width:1150px;margin:auto;padding:40px 24px}header{border-bottom:2px solid #21372f;padding-bottom:24px}.eyebrow{font-size:12px;letter-spacing:2px;font-weight:700}h1{font-size:52px;letter-spacing:-3px;margin:12px 0}h2{font-size:20px;line-height:1.35}p{color:#52655d;line-height:1.5}a{color:#215e44;font-weight:650}header p{max-width:750px}.banner{padding:15px;background:#e5eadd;border-radius:8px;margin:20px 0}.error{background:#ffe8cf}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px;margin:24px 0}article{background:white;border:1px solid #d4ddd3;border-radius:12px;padding:24px}article p{font-size:13px}.row{display:flex;justify-content:space-between;align-items:center}.tag{font-size:12px;background:#f4e7c9;padding:6px 10px;border-radius:20px}.good{background:#daf4bd}.price{font-size:36px;font-weight:700;letter-spacing:-1px}.warning{color:#93501a;min-height:20px}.sample{font-size:13px;color:#7c6950}table{width:100%;border-collapse:collapse}td,th{text-align:left;padding:14px 10px;border-bottom:1px solid #d3dacf;font-size:14px}.table{overflow:auto}footer{margin-top:30px;font-size:13px}small{color:#64766d}@media(max-width:600px){main{padding:24px 16px}h1{font-size:40px}td,th{min-width:100px}}
    ...
    content += '<h2>Your watchlist</h2><p>Edit config.json to change gear, budgets, condition filters, ZIP code, and alert thresholds.</p><div class="table"><table><tr><th>Gear</th><th>Max total</th><th>Used estimate</th><th>Browse</th></tr>' + ''.join(searches) + '</table></div>'
    content += '<footer><p>Benchmarks are editable starter assumptions, not verified sold-market averages. Deal score = percentage below your benchmark. Unknown shipping and seller concerns block alerts. Auction-only listings and common accessory/repair titles are excluded.</p><p>This is a generated report. Refreshing it does not check eBay. Leave the tracker running with --watch for new checks; your computer must remain awake. Nearby browser links filter seller distance; verify actual pickup availability.</p></footer></main></body></html>'
    tmp = path.with_suffix('.tmp')
